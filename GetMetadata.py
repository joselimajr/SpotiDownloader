...
async def get_track_metadata_from_api(session: aiohttp.ClientSession, track_id: str) -> Dict[str, Any]:
...
async def extract_track_metadata(song: Song, track_id: str) -> Dict[str, Any]:
    async with aiohttp.ClientSession() as session:
        try:
            return await get_track_metadata_from_api(session, track_id)
        except Exception as e:
            raise ValueError(f"Failed to fetch track metadata: {str(e)}")

async def fetch_track_metadata_batch(session: aiohttp.ClientSession, track_ids: List[str]) -> List[Dict[str, Any]]:
    tasks = [get_track_metadata_from_api(session, track_id) for track_id in track_ids]
    return await asyncio.gather(*tasks, return_exceptions=True)

async def extract_album_metadata(album: PublicAlbum) -> Dict[str, Any]:
    try:
        ...
        async with aiohttp.ClientSession() as session:
            for page in album.paginate_album():
                ...
                track_metadata = await fetch_track_metadata_batch(session, track_ids)
...
async def extract_playlist_metadata(playlist: PublicPlaylist) -> Dict[str, Any]:
    try:
...
async def get_track_metadata(track_input: str) -> Dict[str, Any]:
    try:
        track_id = extract_spotify_id(track_input)
        song = Song()
        return await extract_track_metadata(song, track_id)
    except Exception as e:
        raise ValueError(f"Error getting track metadata: {str(e)}")

async def get_album_metadata(album_input: str) -> Dict[str, Any]:
    try:
        album_id = extract_spotify_id(album_input)
        album = PublicAlbum(album_id)
        return await extract_album_metadata(album)
    except Exception as e:
        raise ValueError(f"Error getting album metadata: {str(e)}")

async def get_playlist_metadata(playlist_input: str) -> Dict[str, Any]:
    try:
        playlist_id = extract_spotify_id(playlist_input)
        playlist = PublicPlaylist(playlist_id)
        return await extract_playlist_metadata(playlist)
    except Exception as e:
